...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        char_inds = self.words2charindices(sents)
        padded_chars = pad_sents_char(char_inds, self.char2id['<pad>'])
        char_vars = torch.tensor(padded_chars, dtype=torch.long, device=device)
        return char_vars.permute(1,0,2) #shape: (max sentence length, batch size, max word length)
        ### END YOUR CODE

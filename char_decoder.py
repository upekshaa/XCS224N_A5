...
class CharDecoder(nn.Module):
    def __init__(self, hidden_size, char_embedding_size=50, target_vocab=None):
        ...
        self.decoderCharEmb = nn.Embedding(num_embeddings=len(target_vocab.char2id), embedding_dim=char_embedding_size,
                                           padding_idx=self.padding_idx)
        self.charDecoder = nn.LSTM(input_size=self.char_embedding_size, hidden_size=hidden_size)
        self.char_output_projection = nn.Linear(in_features=hidden_size, out_features=len(self.target_vocab.char2id))
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        return loss(scores.permute(1,2,0), char_sequence[1:].transpose(1,0))
        ### END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        current_char = torch.tensor([[start_idx]*batch_size], device=device) #'<START>'
        ht, hc = initialStates
        ...
        for f in output_words:
            word = ''
            for b in f:
                if b==end_idx:
                    break
                word += self.target_vocab.id2char[b]
            decoded_words+=[word]
